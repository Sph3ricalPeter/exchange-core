...
public final class RiskEngine implements WriteBytesMarshallable {
  ...
  public RiskEngine(
    ...
    if (Long.bitCount(numShards) != 1) {
      throw new IllegalArgumentException(
          "Invalid number of shards " + numShards + " - must be power of 2");
    }
    ...
    final OrdersProcessingConfiguration ordersProcCfg =
        exchangeConfiguration.getOrdersProcessingCfg();
    this.cfgIgnoreRiskProcessing =
        ordersProcCfg.getRiskProcessingMode()
            == OrdersProcessingConfiguration.RiskProcessingMode.NO_RISK_PROCESSING;
    this.cfgMarginTradingEnabled =
        ordersProcCfg.getMarginTradingMode()
            == OrdersProcessingConfiguration.MarginTradingMode.MARGIN_TRADING_ENABLED;
  }

  /**
   * Pre-process command handler 1. MOVE/CANCEL commands ignored, for specific uid marked as valid
   * for matching engine 2. PLACE ORDER checked with risk ending for specific uid 3. ADD USER,
   * BALANCE_ADJUSTMENT processed for specific uid, not valid for matching engine 4. BINARY_DATA
   * commands processed for ANY uid and marked as valid for matching engine TODO which handler
   * marks? 5. RESET commands processed for any uid
   *
   * @param cmd - command
  ...
  public boolean preProcessCommand(final long seq, final OrderCommand cmd) {
    switch (cmd.command) {
      ...
      case ADD_USER:
        if (uidForThisHandler(cmd.uid)) {
          cmd.resultCode =
              userProfileService.addEmptyUserProfile(cmd.uid, cmd.feeZone)
      ...
      case BINARY_DATA_QUERY:
        binaryCommandsProcessor.acceptBinaryFrame(
            cmd); // ignore return result, because it should be set by MatchingEngineRouter
        if (shardId == 0) {
      ...
      case PERSIST_STATE_RISK:
        final boolean isSuccess =
            serializationProcessor.storeData(
                cmd.orderId,
                seq,
                cmd.timestamp,
                ISerializationProcessor.SerializedModuleType.RISK_ENGINE,
  ...
  private CommandResultCode adjustBalance(
    ...
    final CommandResultCode res =
        userProfileService.balanceAdjustment(uid, currency, amountDiff, fundingTransactionId);
  ...
  private void handleBinaryMessage(BinaryDataCommand message) {
    if (message instanceof BatchAddSymbolsCommand) {
      ...
      final IntObjectHashMap<CoreSymbolSpecification> symbols =
          ((BatchAddSymbolsCommand) message).getSymbols();
      symbols.forEach(
          spec -> {
      ...
      ((BatchAddAccountsCommand) message)
          .getUsers()
          .forEachKeyValue(
              (uid, accounts) -> {
                if (userProfileService.addEmptyUserProfile(
                    uid, ((BatchAddAccountsCommand) message).getFeeZone())) {
                  // TODO: change fundingTransactionId to something useful - eg. generate sequence
                  // for transactions
                  accounts.forEachKeyValue(
                      (cur, bal) ->
                          adjustBalance(
  ...
  private CommandResultCode placeOrderRiskCheck(final OrderCommand cmd) {
    ...
    final CoreSymbolSpecification spec =
        symbolSpecificationProvider.getSymbolSpecification(cmd.symbol);
    if (spec == null) {
  ...
  private CommandResultCode placeOrder(
      final OrderCommand cmd, final UserProfile userProfile, final CoreSymbolSpecification spec) {
    ...
    if (spec.type == SymbolType.CURRENCY_EXCHANGE_PAIR) {
      ...
      SymbolPositionRecord position =
          userProfile.positions.get(spec.symbolId); // TODO getIfAbsentPut?
      if (position == null) {
  ...
  private CommandResultCode placeExchangeOrder(
    ...
    if (cfgMarginTradingEnabled) {
      for (final SymbolPositionRecord position : userProfile.positions) {
        if (position.currency == currency) {
          ...
          final CoreSymbolSpecification spec2 =
              symbolSpecificationProvider.getSymbolSpecification(recSymbol);
          // add P&L subtract margin
          freeFuturesMargin +=
              (position.estimateProfit(spec2, lastPriceCache.get(recSymbol))
                  - position.calculateRequiredMarginForFutures(spec2));
        }
      }
    }
    ...
    if (cmd.action == OrderAction.BID) {
      ...
      if (cmd.orderType == OrderType.FOK_BUDGET || cmd.orderType == OrderType.IOC_BUDGET) {
        ...
        orderHoldAmount =
            CoreArithmeticUtils.calculateAmountBidTakerFee(
                size, cmd.reserveBidPrice, spec, userProfile.feeZone);
        if (logDebug)
          log.debug(
              // Math.round((1 + userProfile.feeZone.takerFeeFraction) * size * (price *
              // spec.quoteScaleK + spec.takerBaseFee));
              "hold amount buy {} = {} * {} * ( {} * {} + {} )",
              orderHoldAmount,
              1 + userProfile.feeZone.takerFeeFraction,
              size,
      ...
      if (cmd.price * spec.quoteScaleK < spec.takerBaseFee) {
        // log.debug("cmd.price {} * spec.quoteScaleK {} < {} spec.takerFee", cmd.price,
    ...
    if (logDebug) {
      log.debug(
          "R1 uid={} : orderHoldAmount={} vs serProfile.accounts.get({})={} + freeFuturesMargin={}",
          userProfile.uid,
          orderHoldAmount,
          currency,
          userProfile.accounts.get(currency),
          freeFuturesMargin);
    }
    ...
    if (!canPlace) {
      ...
      //            log.warn("orderAmount={} > userProfile.accounts.get({})={}", orderAmount,
      // currency, userProfile.accounts.get(currency));
      return CommandResultCode.RISK_NSF;
    } else {
      return CommandResultCode.VALID_FOR_MATCHING_ENGINE;
    }
  ...
  /**
   * Checks: 1. Users account balance 2. Margin 3. Current limit orders
   *
   * <p>NOTE: Current implementation does not care about accounts and positions quoted in different
   * currencies
   *
   ...
   */
  private boolean canPlaceMarginOrder(
      final OrderCommand cmd,
      final UserProfile userProfile,
      final CoreSymbolSpecification spec,
      final SymbolPositionRecord position) {

    final long newRequiredMarginForSymbol =
    ...
    //                newRequiredMarginForSymbol, position.currency,
    // userProfile.accounts.get(position.currency), freeMargin);
  ...
  public boolean handlerRiskRelease(final long seq, final OrderCommand cmd) {
    ...
    if (mte != null && mte.eventType != MatcherEventType.BINARY_EVENT) {
      ...
      if (spec.type == SymbolType.CURRENCY_EXCHANGE_PAIR) {
        ...
        final SymbolPositionRecord takerSpr =
            (takerUp != null) ? takerUp.getPositionRecordOrThrowEx(symbol) : null;
        do {
    ...
    if (marketData != null && cfgMarginTradingEnabled) {
      final RiskEngine.LastPriceCacheRecord record =
          lastPriceCache.getIfAbsentPut(symbol, RiskEngine.LastPriceCacheRecord::new);
  ...
  private void handleMatcherEventMargin(
    ...
    if (takerUp != null) {
      if (ev.eventType == MatcherEventType.TRADE) {
        ...
        final long fee = spec.takerBaseFee * sizeOpen;
        ...
        fees.addToValue(spec.quoteCurrency, fee);
          ...
          || ev.eventType == MatcherEventType.REDUCE) {
    ...
    if (ev.eventType == MatcherEventType.TRADE && uidForThisHandler(ev.matchedOrderUid)) {
      ...
      long sizeOpen =
          makerSpr.updatePositionForMarginTrade(takerAction.opposite(), ev.size, ev.price);
      final long fee = spec.makerBaseFee * sizeOpen;
  ...
  private void handleMatcherRejectReduceEventExchange(
    ...
    if (takerSell) {
      ...
      taker.accounts.addToValue(
          spec.baseCurrency, CoreArithmeticUtils.calculateAmountAsk(ev.size, spec));
  ...
  private void handleMatcherEventsExchangeBuy(
    ...
    if (taker != null) {
      ...
      taker.accounts.addToValue(
          quoteCurrency, (takerSizePriceHeldSum - takerSizePriceSum) * spec.quoteScaleK);
