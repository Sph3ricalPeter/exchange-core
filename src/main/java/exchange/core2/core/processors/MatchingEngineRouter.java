...
public final class MatchingEngineRouter implements WriteBytesMarshallable {
  ...
  public MatchingEngineRouter(
    ...
    if (exchangeCfg.getInitStateCfg().fromSnapshot() && Files.exists(snapshotPath)) {
      final DeserializedData deserialized =
          serializationProcessor.loadData(
              ...
              bytesIn -> {
                ...
                final BinaryCommandsProcessor bcp =
                ...
                final IntObjectHashMap<IOrderBook> ob =
                    SerializationUtils.readIntHashMap(
                        bytesIn,
                        bytes -> IOrderBook.create(bytes, objectsPool, eventsHelper, loggingCfg));

                return DeserializedData.builder()
                    .binaryCommandsProcessor(bcp)
                    .orderBooks(ob)
                    .build();
              });
  ...
  public void processOrder(long seq, OrderCommand cmd) {
    ...
    if (command == OrderCommandType.MOVE_ORDER
    ...
    } else if (command == OrderCommandType.BINARY_DATA_QUERY
      ...
      final boolean isSuccess =
          serializationProcessor.storeData(
              ...
              shardId,
              this);
      // Send ACCEPTED because this is a first command in series. Risk engine is second - so it will
      // return SUCCESS
  ...
  private void addSymbol(final CoreSymbolSpecification spec) {
    ...
    if (orderBooks.get(spec.symbolId) == null) {
      orderBooks.put(
          spec.symbolId, orderBookFactory.create(spec, objectsPool, eventsHelper, loggingCfg));
    } else {
      log.warn(
          "OrderBook for symbol id={} already exists! Can not add symbol: {}", spec.symbolId, spec);
    }
  }
  ...
  private void processMatchingCommand(final OrderCommand cmd) {
    ...
    if (orderBook == null) {
      ...
      // posting market data for risk processor makes sense only if command execution is successful,
      // otherwise it will be ignored (possible garbage from previous cycle)
  ...
  private Path resolveSnapshotPath(
      long snapshotId, ISerializationProcessor.SerializedModuleType type, int instanceId) {
    return folder.resolve(
        String.format("%s_snapshot_%d_%s%d.ecs", exchangeId, snapshotId, type.getCode(), instanceId));
  }
