...
public final class OrderBookNaiveImpl implements IOrderBook {
  ...
  public OrderBookNaiveImpl(
      final CoreSymbolSpecification symbolSpec, final LoggingConfiguration loggingCfg) {
  ...
  public OrderBookNaiveImpl(final BytesIn bytes, final LoggingConfiguration loggingCfg) {
    ...
    this.bidBuckets =
        SerializationUtils.readLongMap(
            bytes, () -> new TreeMap<>(Collections.reverseOrder()), OrdersBucketNaive::new);
    ...
    askBuckets
        .values()
        .forEach(bucket -> bucket.forEachOrder(order -> idMap.put(order.orderId, order)));
    bidBuckets
        .values()
        .forEach(bucket -> bucket.forEachOrder(order -> idMap.put(order.orderId, order)));
  ...
  private void newOrderMatchIoc(final OrderCommand cmd) {
    ...
    final long filledSize =
        tryMatchInstantly(cmd, subtreeForMatching(cmd.action, cmd.price), 0, cmd);
  ...
  private Optional<Long> checkBudgetToFill(
      long size, final SortedMap<Long, OrdersBucketNaive> matchingBuckets) {
  ...
  private SortedMap<Long, OrdersBucketNaive> subtreeForMatching(
      final OrderAction action, final long price) {
    return (action == OrderAction.ASK ? bidBuckets : askBuckets).headMap(price, true);
  }

  /**
   * Match the order instantly to specified sorted buckets map Fully matching orders are removed
   * from orderId index Should any trades occur - they sent to tradesConsumer
   *
  ...
  private long tryMatchInstantly(
    ...
    for (final OrdersBucketNaive bucket : matchingBuckets.values()) {
      ...
      final OrdersBucketNaive.MatcherResult bucketMatchings =
          bucket.match(sizeLeft, activeOrder, eventsHelper);
  ...
  public CommandResultCode cancelOrder(OrderCommand cmd) {
    ...
    if (ordersBucket == null) {
      ...
      throw new IllegalStateException(
          "Can not find bucket for order price=" + price + " for order " + order);
    }
  ...
  public CommandResultCode moveOrder(OrderCommand cmd) {
    ...
    final OrdersBucketNaive anotherBucket =
  ...
  public List<Order> findUserOrders(final long uid) {
    ...
    Consumer<OrdersBucketNaive> bucketConsumer =
        bucket ->
            bucket.forEachOrder(
                order -> {
                  if (order.uid == uid) {
  ...
  public int getOrdersNum(OrderAction action) {
    final NavigableMap<Long, OrdersBucketNaive> buckets =
        action == OrderAction.ASK ? askBuckets : bidBuckets;
    return buckets.values().stream().mapToInt(OrdersBucketNaive::getNumOrders).sum();
    //        int askOrders =
    // askBuckets.values().stream().mapToInt(OrdersBucketNaive::getNumOrders).sum();
    //        int bidOrders =
    // bidBuckets.values().stream().mapToInt(OrdersBucketNaive::getNumOrders).sum();
